...
class DEITB(MIFGSM):
    ...
    def _register_model(self):
        ...
        if self.model_name in ['deit_base_distilled_patch16_224' ]: 

            for i in [0,1,5,10,11]:
                self.model.blocks[i].attn.attn_drop.register_forward_hook(partial(diverse_attn_map, attn_map_change_range=30))  #  

            for i in range(12):
                self.model.blocks[i].attn.register_forward_hook(partial(cross_iter_emb_momentum, scale=0.7, mom_emb_decay=0.2))

            for i in range(12):
                self.model.blocks[i].mlp.register_forward_hook(partial(cross_iter_emb_momentum, scale=0.7, mom_emb_decay=0.2))
