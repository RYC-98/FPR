...
class TGR(MIFGSM):
    ...
    def _register_model(self):
        ...
        def attn_tgr(module, grad_in, grad_out, gamma):  
            ...
            if self.model_name in ['vit_base_patch16_224', 'visformer_small', 'visformer_tiny', 'pit_b_224', 'pit_ti_224']:
                B,C,H,W = grad_in[0].shape  
                out_grad_cpu = out_grad.data.clone().cpu().numpy().reshape(B,C,H*W) # [B,C,H*W]
        ...
        def v_tgr(module, grad_in, grad_out, gamma):
            ...
            if self.model_name in ['visformer_small', 'visformer_tiny']:    
                B,C,H,W = grad_in[0].shape                
            ...
            if self.model_name in ['vit_base_patch16_224', 'pit_b_224', 'pit_ti_224', 'cait_s24_224']:
                c = grad_in[0].shape[2]           
        ...
        def mlp_tgr(module, grad_in, grad_out, gamma):
            ...
            # grad_in[0]: torch.Size([1, 1536, 14, 14])  grad_in[1]: torch.Size([384, 1536, 1, 1])  grad_in[2]：None
            if self.model_name in ['visformer_small', 'visformer_tiny']:
                B,C,H,W = grad_in[0].shape 
                out_grad_cpu = out_grad.data.clone().cpu().numpy().reshape(B,C,H*W)
                max_all = np.argmax(out_grad_cpu[0,:,:], axis = 1)   
            ...
            if self.model_name in ['vit_base_patch16_224', 'pit_b_224', 'pit_ti_224', 'cait_s24_224', 'resnetv2_101']:
                c = grad_in[0].shape[2] 
                out_grad_cpu = out_grad.data.clone().cpu().numpy()
