...
class PNA_PatchOut(MIFGSM):
    ...
    def __init__(self, model_name, epsilon=16/255, alpha=1.6/255, epoch=10, decay=1., targeted=False, gamma=0.2, random_start=False, norm='linfty', loss='crossentropy', device=None, 
                 attack='pna-patchout', ablation_study='1,0,0', **kwargs):
    ...
    def forward(self, data, label, **kwargs):
        ...
        for epoch_idx in range(self.epoch):
            # Obtain the output
            delta_patchout = self._generate_samples_for_interactions(delta, epoch_idx) 
            ...
            # Add l2 norm
            loss += self.lamb * torch.norm(delta, p=2) # self.lamb = 0.1
    ...
    def _register_model(self):
        ...
        drop_hook_func = partial(attn_drop_mask_grad, gamma=0) # 将 gamma 参数固定为 0
